 ...
 * Copyright (c) 2025 Richard Gracik - Morc
 *          port from Rockbox to ProlinOS and PAX S920
...
unsigned short palette[256];           /* Color palette */
...
void decodeROMs(void)
{
    ...
    for( i=0; i<256; i++ ) {
        ...
        palette[i] = RGB((unsigned char) (c),(unsigned char) (c >> 4),(unsigned char) (c >> 18));
