...
void scan_dir_apps(AppList *list, const char* base_path) {
    ...
    printf("[MAIN] Scanning app folder '%s'...\n", apps_path);
    if (access(apps_path, F_OK) != 0) {
        printf("[W] App folder '%s' does not exist\n", apps_path);
        return;
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (entry->d_type == DT_DIR && strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0) {
            is_empty = 0;
            printf("[MAIN] Found app: %s\n", entry->d_name);
            AppMetadata meta = parseFile(apps_path, entry->d_name);

            if (meta.is_valid) { 
                printf("[MAIN] App metadata parsed successfully.\n");
...
int _init()
{
    ...
    if (access("/data/app/MAINAPP/apps/", F_OK) != 0) {
        printf("[MAIN] App folder does not exist, attempting to create one\n");
